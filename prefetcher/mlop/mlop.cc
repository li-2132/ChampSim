  ...
  void prefetch(CACHE* cache, uint64_t block_number)
  {
    ...
    for (int d = 0; d < PF_DEGREE; d += 1) {
      for (auto& cur_pf_offset : this->pf_offset[d]) {
        ...
        if (this->is_inside_zone(offset_to_prefetch) && cache->PQ.occupancy() < cache->PQ.max_size()
            && cache->PQ.occupancy() + cache->MSHR.size() < cache->MSHR.max_size() - 1) {
...
uint32_t CACHE::l1d_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  if (cache_hit == 1) {
    ...
    if (block[set * NUM_WAY + way].prefetch == 1)
      prefetch_hit = true;
  }
